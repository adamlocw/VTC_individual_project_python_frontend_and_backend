...
class firstMACD_Strategy(bt.Strategy):

    def __init__(self):
        self.macd = bt.indicators.MACD(self.data, period_me1=12, period_me2=26, period_signal=9) #12, 26, 9 - 5, 35, 5
...
dataframe=pd.read_csv('ETHUSD_1d_k_2022only.csv')

dataframe['datetime'] = pd.to_datetime(dataframe['time'])
dataframe.set_index('datetime', inplace=True)
dataframe['openinterest']=0

brf_daily = bt.feeds.PandasData(dataname=dataframe, fromdate=datetime.datetime(2022,1,1,0,0),todate=datetime.datetime(2022,5,10,0,0))
...
cerebro.addstrategy(firstMACD_Strategy)
